...
async function search(value: string) {
  ...
  try {
    const { data } = await api.get<Medecin[]>("/medecins", {
...
const selectedMedecin = computed(
  () => results.value.find((m) => m.id === selectedId.value) ?? null,
);
...
// CalendarDate -> "YYYY-MM-DD" sans passer par un objet Date 
// (donc sans risque de décalage de fuseau).
...
const dateStr = computed(() =>
  dateValue.value ? calendarDateToStr(dateValue.value) : "",
);
...
watch(dateStr, async (value) => {
  ...
  try {
    const { data } = await api.get<Creneau[]>(
      `/medecins/${selectedMedecin.value.id}/creneaux`,
      {
        params: { date: value },
      },
    );
...
<template>
  <UCard
    ...
    :ui="{ body: 'flex min-h-0 flex-1 flex-col gap-4' }"
  >
    <UInput
      v-model="query"
      :loading="searching"
    ...
    <div v-if="results.length" class="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div
        ...
        class="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto border-t pt-4"
      >
        <div>
        ...
        <div v-if="dateStr" class="border-t pt-4">
          <p
            v-if="!loadingCreneaux && creneaux.length === 0"
          ...
          <div v-if="creneaux.length" class="flex flex-wrap gap-2">
            <UButton
              ...
              variant="outline"
              :color="
                c.disponible && !estPasse(c.date_heure) ? 'primary' : 'error'
              "
