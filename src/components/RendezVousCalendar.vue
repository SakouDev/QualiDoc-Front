...
function dayKeyFromCalendarDate(day: {
  year: number;
  month: number;
  day: number;
}): string {
...
const selectedKey = computed(() =>
  selected.value ? dayKeyFromCalendarDate(selected.value) : null,
);
...
<template>
  <UCard variant="subtle" class="bg-white">
    <div>
      ...
      <div
        v-if="selected"
        class="mt-4 flex flex-col divide-y divide-default border-t pt-4"
      >
        <p v-if="selectedDayRdv.length === 0" class="text-sm text-muted">
          Rien ce jour-là.
        </p>
        <div
        ...
        >
          <div>
            <p class="text-sm font-medium">
              {{ rdv.date_heure.slice(11, 16) }} — {{ rdv.medecin_prenom }}
              {{ rdv.medecin_nom }}
            </p>
            <p class="text-xs text-muted">{{ rdv.specialite_nom }}</p>
          </div>
          <div class="flex items-center gap-2">
            <UBadge :color="statutColor(rdv.statut)" variant="subtle">{{
              statutLabel(rdv.statut)
            }}</UBadge>
            <UButton
              v-if="
                rdv.statut === 'confirme' &&
                new Date(rdv.date_heure.replace(' ', 'T')) > new Date()
              "
              size="xs"
              color="error"
              variant="outline"
